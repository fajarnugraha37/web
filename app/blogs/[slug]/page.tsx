    ...
    <article className="py-12 md:py-20 max-w-4xl mx-auto">
      <div className="mb-8 border-b border-border pb-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <time className="text-accent-secondary font-mono text-sm bg-accent-secondary/10 px-2 py-1">
            [ TS: {postData.date} ]
          </time>
          <div className="flex gap-2">
            {postData.tags.map((t: string) => (
              <span
                key={t}
                className="text-xs uppercase tracking-widest text-muted-foreground border border-border px-2 py-1 bg-card"
              >
                {t}
              </span>
            ))}
