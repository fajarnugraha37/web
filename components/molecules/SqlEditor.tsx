...
export function SqlEditor({ onExecute, isLoading, disabled, history, onClearHistory }: SqlEditorProps) {
  ...
  const handleExecute = useCallback(() => {
    ...
    setHistoryIndex(-1);
    onExecute(query);
  }, [query, isLoading, disabled, onExecute]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    // Command History Navigation (Newest to Oldest)
    if (e.key === "ArrowUp") {
      if (history.length > 0) {
        e.preventDefault();
  ...
  return (
    <>
      ...
      <ConfirmationModal 
        ...
        title="Wipe Command History"
        message="This operation will permanently purge all recorded SQL execution logs from the local buffer. Proceed?"
        variant="destructive"
        confirmLabel="PURGE_HISTORY"
      />
    </>
  );
}
